...
import { Activity, Database, Server, AlertTriangle, RefreshCw, ChevronLeft, ChevronRight } from 'lucide-react'
...
export function DashboardView() {
    ...
    const summaryCards = [
        { title: "Total Sessions", value: metrics?.sessions?.total || "...", icon: Activity, description: `${metrics?.sessions?.active || 0} active sessions` },
        { title: "DB Architecture", value: connection.db_type || "Unknown", icon: Database, description: connection.role || "No role info" },
        ...
        { title: "Log Mode", value: connection.log_mode || "Unknown", icon: AlertTriangle, description: connection.is_rac ? "RAC Enabled" : "Single Instance", urgent: connection.log_mode === 'NOARCHIVELOG' },
    ]
    ...
    return (
        <MainLayout>
            <div className="p-4 space-y-4 overflow-hidden h-full flex flex-col bg-[#e0e4e8]"> {/* Windows-ish gray background */}
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                            ...
                            {summaryCards.map((card) => (
                                <Card key={card.title}>
                                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                        ...
                                        <card.icon className={`h-4 w-4 ${card.urgent ? 'text-red-500' : 'text-muted-foreground'}`} />
                                    </CardHeader>
                                    <CardContent>
                        ...
                        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
                            ...
                            <Card className="col-span-3">
                                ...
                                <CardContent>
                                    <div className="text-sm text-muted-foreground text-center py-10">
                                        Wait events breakdown will go here.
                                    </div>
                                </CardContent>
                            </Card>
                        </div>
                ...
                {activeTab === "sga" && (
                    <div className="flex-1 grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1 gap-2 max-w-sm mx-auto w-full h-full content-start animate-in fade-in slide-in-from-bottom-2 duration-300">
                        {/* SGA Panels Column - Matching Screenshot Vertical Layout */}
                        <div className="flex flex-col gap-1 h-full min-h-[600px] border-l-4 border-l-blue-200/50 pl-2 py-2">
                            <h2 className="text-center font-bold text-blue-400/50 text-xl tracking-widest uppercase mb-2">SGA</h2>

                            {/* Panel 1: Block Buffers */}
                            <div className="h-28">
                                <SgaPanel
                        ...
                        {/* Detail Panel */}
                        <div className="mt-4 flex-1 bg-white border border-blue-200 rounded-md shadow-sm p-4 animate-in fade-in slide-in-from-bottom-2">
                            <h3 className="text-sm font-bold text-slate-700 mb-2 uppercase tracking-tight flex items-center gap-2">
                            ...
                            <div className="overflow-auto border rounded-sm h-[300px]">
