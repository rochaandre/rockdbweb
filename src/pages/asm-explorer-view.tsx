...
export function AsmExplorerView() {
    const [activeTab, setActiveTab] = usePersistentState('asm', 'activeTab', 'groups')
    const hasDiskGroups = MOCK_ASM_DATA.diskGroups.length > 0
            ...
            <div className="flex flex-col h-full bg-background overflow-hidden p-6 gap-6">
                ...
                {!hasDiskGroups ? (
                    <div className="flex-1 flex flex-col items-center justify-center text-center p-8 space-y-4">
                        <div className="size-16 rounded-full bg-muted flex items-center justify-center text-muted-foreground opacity-50">
                            <HardDrive className="size-8" />
                        </div>
                        <div className="max-w-md">
                            <h2 className="text-xl font-bold">No Disk Groups Found</h2>
                            <p className="text-muted-foreground mt-2">
                                No ASM disk groups were detected on this instance.
                                ASM Explorer is only available when valid disk groups are in use.
                            </p>
                        </div>
                    </div>
                ) : (
