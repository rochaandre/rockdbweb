...
export function RedoLogView() {
    ...
    const [redoMembers, setRedoMembers] = useState<any[]>([])
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            try {
                const membersRes = await fetch(`${API_URL}/storage/redo/members`)
                if (membersRes.ok) setRedoMembers(await membersRes.json())
            } catch (err) {
                console.error('Error fetching redo members:', err)
            }
        } catch (err) {
    ...
    const generateResizeScript = (size: string) => {
        const firstMember = redoMembers[0]?.member || '/u02/oradata/DB/redo01.log';
        const basePath = firstMember.substring(0, firstMember.lastIndexOf('/') + 1);

        return (`-- Resize script generated for ${basePath}
-- Note: log_file_name_convert is only needed for standby or clone, but shown as requested
ALTER SYSTEM SET log_file_name_convert='${basePath}','${basePath}' SCOPE=SPFILE;

-- Add new larger groups
ALTER DATABASE ADD LOGFILE GROUP 4 ('${basePath}redo04a.log','${basePath}redo04b.log') SIZE ${size}M;
ALTER DATABASE ADD LOGFILE GROUP 5 ('${basePath}redo05a.log','${basePath}redo05b.log') SIZE ${size}M;
ALTER DATABASE ADD LOGFILE GROUP 6 ('${basePath}redo06a.log','${basePath}redo06b.log') SIZE ${size}M;
ALTER DATABASE ADD LOGFILE GROUP 7 ('${basePath}redo07a.log','${basePath}redo07b.log') SIZE ${size}M;
ALTER DATABASE ADD LOGFILE GROUP 8 ('${basePath}redo08a.log','${basePath}redo08b.log') SIZE ${size}M;

-- Switch and Archive to move current logs
...
-- Drop old groups (ensure they are INACTIVE/ARCHIVED first)
-- ALTER DATABASE DROP LOGFILE GROUP 1;
-- ALTER DATABASE DROP LOGFILE GROUP 2;
-- ALTER DATABASE DROP LOGFILE GROUP 3;

-- Re-add old groups with new size
-- ALTER DATABASE ADD LOGFILE GROUP 1 ('${basePath}redo01a.log','${basePath}redo01b.log') SIZE ${size}M;
-- ALTER DATABASE ADD LOGFILE GROUP 2 ('${basePath}redo02a.log','${basePath}redo02b.log') SIZE ${size}M;
-- ALTER DATABASE ADD LOGFILE GROUP 3 ('${basePath}redo03a.log','${basePath}redo03b.log') SIZE ${size}M;
                    ...
                    <Tabs value={activeTab} onValueChange={setActiveTab} className="h-full flex flex-col">
                        ...
                        <div className="flex-1 overflow-auto bg-slate-50 p-4">
                            <TabsContent value="groups" className="mt-0 h-full space-y-4">
                                ...
                                <RedoManager groups={groups} members={redoMembers} history={history} onRefresh={fetchData} />
                            </TabsContent>
                                            ...
                                            {groups.map(g => (
                                                <div key={g['group#']} className="p-3 border rounded bg-white flex justify-between items-start">
                                                    <div className="flex-1 min-w-0">
                                                        ...
                                                        <div className="text-[10px] font-mono whitespace-pre-wrap break-all opacity-80">
                                                            {redoMembers.filter(m => m['group#'] === g['group#']).map(m => m.member).join('\n')}
                                                        </div>
                                                    </div>
