...
// --- Redo Manager ---
export function RedoManager({ groups = [], members = [], history = [], onRefresh }: { groups: any[], members?: any[], history?: any[], onRefresh?: () => void }) {
    const { connection } = useApp()
    const isPdb = connection.db_type === 'PDB'

    // Data for charts
    const currentSeq = groups.find(g => g.status === 'CURRENT')?.['sequence#'] || 0
    const totalGroups = groups.length
    const avgSizeMB = groups.length > 0 ? (groups.reduce((acc, g) => acc + g.size_mb, 0) / groups.length).toFixed(0) : 0

    // Switch Rate Calculation (sum of all switches in the last day divided by 24)
    ...
    if (latestDay) {
        ...
        for (let i = 0; i < 24; i++) {
            ...
            const val = parseInt(latestDay[`h${h}`]) || 0
            dailyTotal += val
            hourlyData.push({ hour: h, count: val })
        }
        switchRate = Math.round(dailyTotal / 24)
    } else {
        hourlyData = Array.from({ length: 24 }).map((_, i) => ({ hour: i.toString().padStart(2, '0'), count: 0 }))
    }

    // Not Archived Calculation
    const notArchivedCount = groups.filter(g => g.archived === 'NO').length
    const notArchivedPct = totalGroups > 0 ? Math.round((notArchivedCount / totalGroups) * 100) : 0
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-full">
            <div className="space-y-4">
                <div className="rounded-md border border-border bg-surface">
                    ...
                    {groups.map((g, i) => (
                        <div key={i} className="border-b border-border last:border-0 hover:bg-muted/10">
                            <div className="grid grid-cols-8 gap-4 p-3 text-sm items-center">
                            ...
                            {/* Member files */}
                            <div className="px-3 pb-2 -mt-1 ml-8 space-y-0.5">
                                {members.filter(m => m['group#'] === g['group#']).map((m, mi) => (
                                    <div key={mi} className="text-[10px] font-mono text-muted-foreground/60 truncate" title={m.member}>
                                        {m.member}
                                    </div>
                                ))}
                            </div>
                ...
                <div className="rounded-md border border-border bg-muted/20 p-4">
                    <div className="space-y-4">
                        <div className="text-xs font-bold text-muted-foreground uppercase flex justify-between items-center">
                            <span>REDO LOG FILES: {totalGroups} x {avgSizeMB} MB</span>
                            <span className="font-mono">Current seq# {currentSeq}</span>
                        </div>

                        <div className="grid grid-cols-2 gap-4">
                            {/* Switch Rate Chart */}
                            <div className="space-y-2">
                                <div className="text-xs font-medium">Switch Rate {switchRate}/hour</div>
                                <div className="h-24 w-full bg-white/50 border rounded p-1">
                                    <ResponsiveContainer width="100%" height="100%">
                            ...
                            {/* Not Archived Chart */}
                            <div className="space-y-2">
                                <div className="text-xs font-medium">{notArchivedPct}% Not Archived</div>
                                <div className="h-24 w-full bg-white/50 border rounded p-1">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <BarChart data={[{ val: notArchivedPct }]}>
                                            <Bar dataKey="val" fill="#ef4444" radius={[1, 1, 0, 0]} />
