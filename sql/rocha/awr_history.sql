 ...
 *********************************************************************/
set echo off
set feedback off timing off verify off pagesize 100 linesize 200 recsep off
set serveroutput on size unlimited format wrapped trimout on trimspool on
col phv heading "Plan|Hash Value"
col snap_time format a12 truncate heading "Snapshot|Time"
col execs format 999G999G990 heading "Execs"
col lio_per_exec  format 999G999G999G990D00 heading "Avg LIO|Per Exec"
col pio_per_exec  format 999G999G999G990D00 heading "Avg PIO|Per Exec"
col row_per_exec  format 999G999G990D00     heading "Avg ROW|Per Exec"
col cpu_per_exec  format 999G999G990D00000  heading "Avg|CPU (secs)|Per Exec"
col ela_per_exec  format 999G999G990D00000  heading "Avg|Elapsed (secs)|Per Exec"
col wall_per_exec format 999G999G990D0000   heading "Avg|Wall Time (secs)|Per Exec"
col buffer_cache_hit_ratio format 9990D00   heading "Avg|  Buffer Cache|Hit Ratio"
col avg_read_msec format 990D000            heading "Avg|  Elapsed (ms)|Per PIO"
...
--	    nvl(ss.OPTIMIZER_COST,0) cost,
		...
		decode(sum(ss.px_servers_execs_delta),0,sum(ss.executions_delta),sum(ss.px_servers_execs_delta))sum_px_execs
...
begin
	...
	select	decode('&&V_NBR_DAYS','',999,to_number(nvl('&&V_NBR_DAYS','999')))
	into	:v_nbr_days
	from	dual;
	--
	v_errcontext := 'open/fetch get_phv';
	for phv in get_phv(trim('&&V_SQL_ID'), :v_nbr_days) loop
		--
		...
		dbms_output.put_line('|'||
			...
			rpad(trim(to_char(phv.sum_execs,'999G999G990')),12,' ')||
--			rpad(trim(to_char(phv.cost,'999999999')),12,' ')||
			rpad(trim(to_char(phv.sum_buffer_gets,'999G999G999G990')),15,' ')||
			rpad(trim(to_char(phv.sum_disk_reads,'999G999G999G990')),15,' ')||
			rpad(trim(to_char(phv.sum_cpu_time,'999G990D00')),12,' ')||
			rpad(trim(to_char(phv.sum_elapsed_time,'999G999G990D00')),15,' ')||
			rpad(trim(to_char(phv.sum_cpu_time/phv.sum_execs,'999G990D0000')),12,' ')||
			rpad(trim(to_char(phv.sum_elapsed_time/phv.sum_execs,'999G990D0000')),12,' ')||
			rpad(trim(to_char(phv.sum_elapsed_time/phv.sum_px_execs,'999G990D0000')),12,' ')||'|');
			--
...
--		rpad('-',12,'-')||
	...
	for phv in get_phv(trim('&&V_SQL_ID'), :v_nbr_days) loop
		...
		if v_prev_plan_hash_value <> phv.plan_hash_value then
			...
			for s in get_xplan(trim('&&V_SQL_ID'), phv.plan_hash_value) loop
				--
...
compute avg of wall_per_exec on phv
compute avg of buffer_cache_hit_ratio on phv
compute avg of avg_read_msec on phv
...
select	ss.plan_hash_value phv,
	...
	(ss.elapsed_time_delta/1000000)/decode(ss.px_servers_execs_delta,0,decode(ss.executions_delta,0,1,ss.executions_delta),ss.px_servers_execs_delta) wall_per_exec,
	    (100*(1-ss.disk_reads_delta/nullif(ss.buffer_gets_delta, 0))) buffer_cache_hit_ratio,
        (iowait_delta/1000/nullif(disk_reads_delta, 0)) avg_read_msec
from 	dba_hist_snapshot	s,
	dba_hist_sqlstat	ss
where	ss.dbid = s.dbid
and	ss.instance_number = s.instance_number
and	ss.snap_id = s.snap_id
and	ss.sql_id = trim('&&V_SQL_ID')
and	ss.executions_delta > 0
...
compute avg of wall_per_exec on report
compute avg of buffer_cache_hit_ratio on report
compute avg of avg_read_msec on report
